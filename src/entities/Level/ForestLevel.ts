...
export default class ForestLevel extends Component {
  public readonly name = 'ForestLevel';
  ...
  constructor(scene: THREE.Scene, physicsWorld: Ammo.btDiscreteDynamicsWorld) {
    super();
    this.scene = scene;
  ...
  CreateGround(): void {
    ...
    const groundShape = new Ammo.btBoxShape(
      new Ammo.btVector3(this.mapSize / 2, 0.5, this.mapSize / 2)
    );
  ...
  AddTreeCollider(x: number, z: number, scale: number): void {
