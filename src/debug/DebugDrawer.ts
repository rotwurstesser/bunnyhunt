...
export default class DebugDrawer {
  private scene: Scene;
  private world: any;
  private options: DebugDrawerOptions;
  ...
  constructor(scene: Scene, world: any, options: DebugDrawerOptions = {}) {
    this.scene = scene;
    this.world = world;
    this.options = options;
    this.debugDrawMode = options.debugDrawMode || AmmoDebugConstants.DrawWireframe;
    const drawOnTop = (this.debugDrawMode & AmmoDebugConstants.DrawOnTop) !== 0;
  ...
  drawLine(from: number, to: number, color: number): void {
    const heap = Ammo.HEAPF32 as Float32Array;
    ...
    (this.geometry.attributes.position as BufferAttribute).setXYZ(
      this.index,
      fromX,
      fromY,
      fromZ
    );
    (this.geometry.attributes.color as BufferAttribute).setXYZ(this.index++, r, g, b);

    const toX = heap[(to + 0) / 4];
    const toY = heap[(to + 4) / 4];
    const toZ = heap[(to + 8) / 4];
    (this.geometry.attributes.position as BufferAttribute).setXYZ(this.index, toX, toY, toZ);
    (this.geometry.attributes.color as BufferAttribute).setXYZ(this.index++, r, g, b);
  }
  ...
  drawContactPoint(
    ...
    (this.geometry.attributes.position as BufferAttribute).setXYZ(this.index, x, y, z);
    (this.geometry.attributes.color as BufferAttribute).setXYZ(this.index++, r, g, b);
  ...
  draw3dText(_location: number, _textString: number): void {
    //TODO
