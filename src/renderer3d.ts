...
export class Renderer3D {
    ...
    private initInstances() {
        ...
        this.grassMesh = new THREE.InstancedMesh(grassGeo, grassMat, 400000); // Increased limit for density
        this.grassMesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
        this.grassMesh.instanceColor = new THREE.InstancedBufferAttribute(new Float32Array(400000 * 3), 3);
    ...
    private loadModels() {
        ...
        gltfLoader.load('models/bun-bun_buddy.glb', (gltf) => {
            // GLTF returns a Scene. We need the geometry.
            // Usually it's in gltf.scene
            let geometry: THREE.BufferGeometry | null = null;
            gltf.scene.traverse((child) => {
                if ((child as THREE.Mesh).isMesh) {
                    // Take the first mesh we find
            ...
            if (geometry) {
                ...
                if (maxDim > 0) {
                    ...
                    // GLTF is usually Y-up, but let's check rotation. 
                    // Often they are correct, unlike OBJ which can be Z-up.
                    // If it faces wrong way, we rotate.
                    // Assuming standard forward is +Z or -Z. 
                    // Let's try default first, maybe add rotation if needed.
                    // (geometry as THREE.BufferGeometry).rotateY(Math.PI); 
        ...
        objLoader.load('models/fox.obj', (group) => {
            ...
            if (geometry) {
                ...
                const maxDim = Math.max(size.x, size.y, size.z);
                if (maxDim > 0) {
                    const scale = (1.0 / maxDim) * 3; // Fox is larger, size 3
                    (geometry as THREE.BufferGeometry).scale(scale, scale, scale);
                    ...
                    if (cell.plant.type === PlantType.Oak || cell.plant.type === PlantType.Pine) {
                        ...
                        if (cell.plant.isBurnt) {
                            ...
                            if (cell.plant.currentEnergy < cell.plant.maxEnergy * 0.3) {
                                _color.lerp(new THREE.Color(0x8B4513), 0.5);
                            }
                        }
    ...
    private spawnKillEffect(x: number, y: number, world: World) {
        ...
        const cx = x + offset;
        const cz = y + offset;
        const cell = world.cells[Math.floor(y)][Math.floor(x)]; // Ensure int for access logic if needed, though mostly visual
        // Previously: Math.max(0.1, cell.height) * 8 + ... 
        // Let's use the ground height
        let groundY = 0;
        if (cell) groundY = Math.max(0.1, cell.height) * 8;

        // 0. Flash Sphere (The "Kill Animation")
        const sphereGeo = new THREE.SphereGeometry(0.5, 8, 8);
        const sphereMat = new THREE.MeshBasicMaterial({ color: 0xff0000, transparent: true, opacity: 0.6 });
        const sphere = new THREE.Mesh(sphereGeo, sphereMat);
        sphere.position.set(cx, groundY + 2.0, cz); // Higher up
        sphere.userData = { type: 'legacy_kill', life: 1.0 };
        this.effectsGroup.add(sphere);

        // 1. Blood Puddle
        const puddleGeo = new THREE.CircleGeometry(0.6, 12);
